...
// the var_info is a pointer, allowing for dynamic definition of the var_info
// tree
...
// Default value is a "pointer" actually its a offest from the base value, but
...
#define AP_GROUPINFO_FRAME(name, idx, clazz, element, def, frame_flags)        \
  AP_GROUPINFO_FLAGS(name, idx, clazz, element, def,                           \
                     (frame_flags) << AP_PARAM_FRAME_TYPE_SHIFT)

// declare a group var_info line with both flags and frame type mask
#define AP_GROUPINFO_FLAGS_FRAME(name, idx, clazz, element, def, flags,        \
                                 frame_flags)                                  \
  AP_GROUPINFO_FLAGS(name, idx, clazz, element, def,                           \
                     flags | ((frame_flags) << AP_PARAM_FRAME_TYPE_SHIFT))

// declare a group var_info line with a default "pointer"
#define AP_GROUPINFO_FLAGS_DEFAULT_POINTER(name, idx, clazz, element, def)     \
  {                                                                            \
    name, AP_VAROFFSET(clazz, element), {                                      \
      def_value_offset :                                                       \
          AP_VAROFFSET(clazz, element) - AP_VAROFFSET(clazz, def)              \
    },                                                                         \
        AP_PARAM_FLAG_DEFAULT_POINTER, idx, AP_CLASSTYPE(clazz, element)       \
  }
...
// declare a subgroup entry in a group var_info. This is for having another
// arbitrary object as a member of the parameter list of an object
#define AP_SUBGROUPINFO(element, name, idx, thisclazz, elclazz)                \
  {                                                                            \
    name, AP_VAROFFSET(thisclazz, element), {group_info : elclazz::var_info},  \
...
#define AP_SUBGROUPEXTENSION(name, idx, clazz, vinfo)                          \
  {                                                                            \
    name, 0, {group_info : clazz::vinfo}, AP_PARAM_FLAG_NESTED_OFFSET, idx,    \
        AP_PARAM_GROUP                                                         \
  }
...
#define AP_SUBGROUPVARPTR(element, name, idx, thisclazz, var_info)             \
  {                                                                            \
    name, AP_VAROFFSET(thisclazz, element), {group_info_ptr : &var_info},      \
        AP_PARAM_FLAG_POINTER | AP_PARAM_FLAG_INFO_POINTER, idx,               \
        AP_PARAM_GROUP                                                         \
  }

#define AP_GROUPEND                                                            \
  { "", 0, {group_info : nullptr}, 0, 0xFF, AP_PARAM_NONE }

// Vehicle defines for info struct
#define GSCALAR(v, name, def)                                                  \
  {                                                                            \
    name, &AP_PARAM_VEHICLE_NAME.g.v, {def_value : def}, 0,                    \
        Parameters::k_param_##v, AP_PARAM_VEHICLE_NAME.g.v.vtype               \
  }
#define GARRAY(v, index, name, def)                                            \
  {                                                                            \
    name, &AP_PARAM_VEHICLE_NAME.g.v[index], {def_value : def}, 0,             \
        Parameters::k_param_##v##index, AP_PARAM_VEHICLE_NAME.g.v[index].vtype \
  }
#define ASCALAR(v, name, def)                                                  \
  {                                                                            \
    name, (const void *)&AP_PARAM_VEHICLE_NAME.aparm.v, {def_value : def}, 0,  \
        Parameters::k_param_##v, AP_PARAM_VEHICLE_NAME.aparm.v.vtype           \
  }
#define GGROUP(v, name, class)                                                 \
  {                                                                            \
    name, &AP_PARAM_VEHICLE_NAME.g.v, {group_info : class ::var_info}, 0,      \
        Parameters::k_param_##v, AP_PARAM_GROUP                                \
  }
#define GOBJECT(v, name, class)                                                \
  {                                                                            \
    name, (const void *)&AP_PARAM_VEHICLE_NAME.v,                              \
        {group_info : class ::var_info}, 0, Parameters::k_param_##v,           \
        AP_PARAM_GROUP                                                         \
  }
#define GOBJECTPTR(v, name, class)                                             \
  {                                                                            \
    name, (const void *)&AP_PARAM_VEHICLE_NAME.v,                              \
        {group_info : class ::var_info}, AP_PARAM_FLAG_POINTER,                \
        Parameters::k_param_##v, AP_PARAM_GROUP                                \
  }
#define GOBJECTVARPTR(v, name, var_info_ptr)                                   \
  {                                                                            \
    name, (const void *)&AP_PARAM_VEHICLE_NAME.v,                              \
        {group_info_ptr : var_info_ptr},                                       \
...
#define GOBJECTN(v, pname, name, class)                                        \
  {                                                                            \
    name, (const void *)&AP_PARAM_VEHICLE_NAME.v,                              \
        {group_info : class ::var_info}, 0, Parameters::k_param_##pname,       \
        AP_PARAM_GROUP                                                         \
  }
#define PARAM_VEHICLE_INFO                                                     \
  {                                                                            \
    "", (const void *)&AP_PARAM_VEHICLE_NAME,                                  \
        {group_info : AP_Vehicle::var_info}, 0, Parameters::k_param_vehicle,   \
        AP_PARAM_GROUP                                                         \
  }
#define AP_VAREND                                                              \
  { "", nullptr, {group_info : nullptr}, 0, 0, AP_PARAM_NONE }
...
public:
  ...
  struct GroupInfo {
    ...
    union {
      ...
      const struct GroupInfo *
          *group_info_ptr; // when AP_PARAM_FLAG_INFO_POINTER is set in flags
      const float def_value;
      ptrdiff_t
          def_value_offset; // Default value offset from param object, when
                            // AP_PARAM_FLAG_DEFAULT_POINTER is set in flags
    };
    uint16_t flags;
  ...
  static uint32_t group_id(const struct GroupInfo *grpinfo, uint32_t base,
  ...
  void copy_name_info(const struct AP_Param::Info *info,
                      ...
                      const struct GroupNesting &group_nesting, uint8_t idx,
  ...
  void copy_name_token(const ParamToken &token, char *buffer, size_t bufferSize,
                       bool force_scalar = false) const;
  ...
  ///
  /// @param table            pointer to array of defaults_table_struct
  /// structures
  /// @param count            number of elements in table array
  static void set_defaults_from_table(const struct defaults_table_struct *table,
                                      uint8_t count);
  ...
  static AP_Param *find_by_name(const char *name, enum ap_var_type *ptype,
                                ParamToken *token);
  ...
  static bool find_key_by_pointer_group(const void *ptr, uint16_t vindex,
                                        const struct GroupInfo *group_info,
  ...
  static bool get_eeprom_full(void) { return eeprom_full; }
  ...
  /// reoad the hal.util defaults file. Called after pointer parameters have
  /// been allocated
  ///
  static void reload_defaults_file(bool last_pass);

  static void load_object_from_eeprom(const void *object_pointer,
                                      const struct GroupInfo *group_info);
  ...
  */
  ...
  static void setup_object_defaults(const void *object_pointer,
                                    const struct GroupInfo *group_info);
  ...
  static bool find_old_parameter(const struct ConversionInfo *info,
                                 AP_Param *value);

  // convert old vehicle parameters to new object parameters
  static void
  convert_old_parameters(const struct ConversionInfo *conversion_table,
                         uint8_t table_size, uint8_t flags = 0);
  // convert old vehicle parameters to new object parameters with scaling -
  // assumes we use the same scaling factor for all values in the table
  static void
  convert_old_parameters_scaled(const ConversionInfo *conversion_table,
                                uint8_t table_size, float scaler,
                                uint8_t flags);
  ...
  static void
  convert_toplevel_objects(const TopLevelObjectConversion g2_conversions[],
                           uint8_t num_conversions);
  ...
  */
  ...
  enum {
    ...
    CONVERT_FLAG_FORCE =
        2 // store new value even if configured in eeprom already
  };
  static void convert_old_parameter(const struct ConversionInfo *info,
                                    float scaler, uint8_t flags = 0);
  ...
  */
  static bool get_param_by_index(void *obj_ptr, uint8_t idx,
  ...
  static AP_Param *next(ParamToken *token, enum ap_var_type *ptype) {
    return next(token, ptype, false);
  }
  static AP_Param *next(ParamToken *token, enum ap_var_type *ptype,
                        bool skip_disabled, float *default_val = nullptr);
  ...
  static void show(const AP_Param *param, const char *name,
                   enum ap_var_type ptype, AP_HAL::BetterStream *port);

  /// print the value of one variable
  static void show(const AP_Param *param, const ParamToken &token,
                   enum ap_var_type ptype, AP_HAL::BetterStream *port);
  ...
  static bool add_param(uint8_t key, uint8_t param_num, const char *pname,
                        float default_value);
...
private:
   ...
   */
  struct Param_header {
    // to get 9 bits for key we needed to split it into two parts to keep binary
    // compatibility
    uint32_t key_low : 8;
    uint32_t type : 5;
    uint32_t key_high : 1;
  ...
  static void check_group_info(const struct GroupInfo *group_info,
                               uint16_t *total_size, uint8_t max_bits,
                               uint8_t prefix_length);
  static bool duplicate_key(uint16_t vindex, uint16_t key);

  static bool adjust_group_offset(uint16_t vindex,
                                  const struct GroupInfo &group_info,
  ...
  const struct Info *
  find_var_info_group(const struct GroupInfo *group_info, uint16_t vindex,
                      uint32_t group_base, uint8_t group_shift,
                      ptrdiff_t group_offset, uint32_t *group_element,
                      const struct GroupInfo *&group_ret,
                      struct GroupNesting &group_nesting, uint8_t *idx) const;
  const struct Info *find_var_info(uint32_t *group_element,
                                   const struct GroupInfo *&group_ret,
  ...
  find_by_header_group(struct Param_header phdr, void **ptr, uint16_t vindex,
                       const struct GroupInfo *group_info, uint32_t group_base,
                       uint8_t group_shift, ptrdiff_t group_offset);
  static const struct Info *find_by_header(struct Param_header phdr,
                                           void **ptr);
  void add_vector3f_suffix(char *buffer, size_t buffer_size, uint8_t idx) const;
  static AP_Param *find_group(const char *name, uint16_t vindex,
  ...
  static AP_Param *next_group(const uint16_t vindex,
                              const struct GroupInfo *group_info,
  ...
  static float get_default_value(const AP_Param *object_ptr,
   ...
   */
  static bool count_defaults_in_file(const char *filename,
                                     uint16_t &num_defaults);
  static bool count_param_defaults(const volatile char *ptr, int32_t length,
                                   uint16_t &count);
  static bool read_param_defaults_file(const char *filename, bool last_pass,
                                       uint16_t &idx);

  // load a defaults.parm using AP_FileSystem:
  static void load_defaults_file_from_filesystem(const char *filename,
                                                 bool lastpass);
  // load an @ROMFS defaults.parm using ROMFS API:
  static void load_defaults_file_from_romfs(const char *filename,
                                            bool lastpass);

  // load defaults from supplied string:
  static void load_param_defaults(const volatile char *ptr, int32_t length,
                                  bool last_pass);
  ...
  /*
    ...
    true if the parameter was converted from an old parameter value
  */
  bool _convert_parameter_width(ap_var_type old_ptype, float scale_factor,
                                bool bitmask);

  // send a parameter to all GCS instances
  void send_parameter(const char *name, enum ap_var_type param_header_type,
                      uint8_t idx) const;
  ...
  static const struct AP_Param::Info &var_info(uint16_t i) {
    return i < _num_vars_base ? _var_info[i]
                              : _var_info_dynamic[i - _num_vars_base];
  }
  static uint8_t _dynamic_table_sizes[AP_PARAM_MAX_DYNAMIC];
#else
  // simple static var table in flash
  static const struct Info &var_info(uint16_t i) { return _var_info[i]; }
#endif
  ...
  */
  ...
  // support for background saving of parameters. We pack it to reduce memory
  // for the queue
...
///
template <typename T, ap_var_type PT> class AP_ParamTBase : public AP_Param {
public:
...
class AP_ParamT<float, AP_PARAM_FLOAT>
    : public AP_ParamTBase<float, AP_PARAM_FLOAT> {
public:
  ...
  ///
  template <bool X = true> operator const float &() const {
    return this->_value;
  }
...
#define PARAM_SELF_OPER(R, OP)                                                 \
  R operator OP(const AP_ParamT<float, AP_PARAM_FLOAT> &other) const {         \
...
///
template <typename T, ap_var_type PT> class AP_ParamV : public AP_Param {
public:
  static const ap_var_type vtype = PT;
...
AP_PARAMDEF(float, Float,
            AP_PARAM_FLOAT); // defines AP_Float, requires specialization!
...
static_assert(not std::is_convertible<AP_Float, int>::value,
              "illegal conversion possible");

/*
  template class for enum types based on AP_Int8
 */
template <typename eclass> class AP_Enum : public AP_Int8 {
public:
  operator const eclass() const { return (eclass)_value; }
  void set(eclass v) { AP_Int8::set(int8_t(v)); }
  void set_and_save(eclass v) { AP_Int8::set_and_save(int8_t(v)); }
};

template <typename eclass> class AP_Enum16 : public AP_Int16 {
public:
  operator const eclass() const { return (eclass)_value; }
  void set(eclass v) { AP_Int16::set(int16_t(v)); }
  void set_and_save(eclass v) { AP_Int16::set_and_save(int16_t(v)); }
};
