...
*/
uint32_t AP_Param::group_id(const struct GroupInfo *grpinfo, uint32_t base,
                            uint8_t i, uint8_t shift) {
  if (grpinfo[i].idx == 0 && shift != 0 &&
      !(grpinfo[i].flags & AP_PARAM_FLAG_NO_SHIFT)) {
    /*
...
void AP_Param::check_group_info(const struct AP_Param::GroupInfo *group_info,
                                uint16_t *total_size, uint8_t group_shift,
  ...
  for (uint8_t i = 0; (type = group_info[i].type) != AP_PARAM_NONE; i++) {
    ...
    if (type == AP_PARAM_GROUP) {
      ...
      check_group_info(ginfo, total_size, group_shift + _group_level_shift,
                       prefix_length + strlen(group_info[i].name));
      continue;
    }
...
AP_Param::get_group_info(const struct GroupInfo &ginfo) {
 ...
 */
const struct AP_Param::GroupInfo *
AP_Param::get_group_info(const struct Info &info) {
...
bool AP_Param::setup(void) {
  ...
  if (hdr.magic[0] != k_EEPROM_magic0 || hdr.magic[1] != k_EEPROM_magic1 ||
      hdr.revision != k_EEPROM_revision) {
#if AP_PARAM_STORAGE_BAK_ENABLED
    if (hdr2.magic[0] == k_EEPROM_magic0 && hdr2.magic[1] == k_EEPROM_magic1 &&
...
bool AP_Param::initialised(void) { return _var_info != nullptr; }
 ...
 */
bool AP_Param::adjust_group_offset(uint16_t vindex,
  ...
  if (group_info.flags & AP_PARAM_FLAG_POINTER) {
    ...
    // calculate offset that is needed to take base object and adjust for this
    // object
...
AP_Param *param() { return AP_Param::get_singleton(); }
...
const struct AP_Param::Info *AP_Param::find_by_header_group(
    struct Param_header phdr, void **ptr, uint16_t vindex,
    const struct GroupInfo *group_info, uint32_t group_base,
    uint8_t group_shift, ptrdiff_t group_offset) {
  uint8_t type;
  for (uint8_t i = 0; (type = group_info[i].type) != AP_PARAM_NONE; i++) {
    if (type == AP_PARAM_GROUP) {
      // a nested group
      ...
      const struct AP_Param::Info *ret =
          find_by_header_group(phdr, ptr, vindex, ginfo,
    ...
    if (group_id(group_info, group_base, i, group_shift) ==
            phdr.group_element &&
        type == phdr.type) {
...
// return the Info structure and a pointer to the variables storage
const struct AP_Param::Info *AP_Param::find_by_header(struct Param_header phdr,
                                                      void **ptr) {
...
const struct AP_Param::Info *AP_Param::find_var_info_group(
    const struct GroupInfo *group_info, uint16_t vindex, uint32_t group_base,
    uint8_t group_shift, ptrdiff_t group_offset, uint32_t *group_element,
    const struct GroupInfo *&group_ret, struct GroupNesting &group_nesting,
    uint8_t *idx) const {
  ptrdiff_t base;
  if (!get_base(var_info(vindex), base)) {
    return nullptr;
  ...
  for (uint8_t i = 0; (type = group_info[i].type) != AP_PARAM_NONE; i++) {
    ...
    if (type == AP_PARAM_GROUP) {
      ...
      info = find_var_info_group(
          ginfo, vindex, group_id(group_info, group_base, i, group_shift),
          group_shift + _group_level_shift, new_offset, group_element,
          group_ret, group_nesting, idx);
      if (info != nullptr) {
        return info;
      }
      group_nesting.level--;
...
const struct AP_Param::Info *AP_Param::find_var_info(
    uint32_t *group_element, const struct GroupInfo *&group_ret,
    struct GroupNesting &group_nesting, uint8_t *idx) const {
  group_ret = nullptr;
  ...
  for (uint16_t i = 0; i < _num_vars; i++) {
    ...
    if (type == AP_PARAM_GROUP) {
      ...
      info2 = find_var_info_group(group_info, i, 0, 0, 0, group_element,
                                  group_ret, group_nesting, idx);
      if (info2 != nullptr) {
        return info2;
      }
...
// find the info structure for a variable
const struct AP_Param::Info *
AP_Param::find_var_info_token(const ParamToken &token, uint32_t *group_element,
...
bool AP_Param::is_sentinal(const Param_header &phdr) {
  ...
  // to EEPROM
  if (phdr.type == _sentinal_type || get_key(phdr) == _sentinal_key) {
    return true;
  }
...
bool AP_Param::scan(const AP_Param::Param_header *target, uint16_t *pofs) {
  ...
  while (ofs < _storage.size()) {
    ...
    if (phdr.type == target->type && get_key(phdr) == get_key(*target) &&
 ...
 */
void AP_Param::add_vector3f_suffix(char *buffer, size_t buffer_size,
                                   uint8_t idx) const {
...
//
void AP_Param::copy_name_token(const ParamToken &token, char *buffer,
                               size_t buffer_size, bool force_scalar) const {
  uint32_t group_element;
  const struct GroupInfo *ginfo;
  struct GroupNesting group_nesting {};
  uint8_t idx;
  const struct AP_Param::Info *info =
      find_var_info_token(token, &group_element, ginfo, group_nesting, &idx);
  if (info == nullptr) {
    *buffer = 0;
    Debug("no info found");
    return;
  }
  copy_name_info(info, ginfo, group_nesting, idx, buffer, buffer_size,
                 force_scalar);
}
...
void AP_Param::copy_name_info(const struct AP_Param::Info *info,
                              ...
                              uint8_t idx, char *buffer, size_t buffer_size,
  ...
  for (uint8_t i = 0; i < group_nesting.level; i++) {
    ...
    if (len < buffer_size) {
      strncpy(&buffer[len], group_nesting.group_ret[i]->name,
              buffer_size - len);
    }
  }
  if (ginfo != nullptr) {
...
AP_Param *AP_Param::find_group(const char *name, uint16_t vindex,
  ...
  for (uint8_t i = 0; (type = group_info[i].type) != AP_PARAM_NONE; i++) {
    if (type == AP_PARAM_GROUP) {
      if (strncasecmp(name, group_info[i].name, strlen(group_info[i].name)) !=
          0) {
        continue;
      }
      const struct GroupInfo *ginfo = get_group_info(group_info[i]);
      ...
      AP_Param *ap = find_group(name + strlen(group_info[i].name), vindex,
                                new_offset, ginfo, ptype);
      if (ap != nullptr) {
        return ap;
      }
      ...
      *ptype = (enum ap_var_type)type;
      ...
      if (strncmp(name, group_info[i].name, suffix_len) == 0 &&
          name[suffix_len] == '_' &&
          (name[suffix_len + 1] == 'X' || name[suffix_len + 1] == 'Y' ||
           name[suffix_len + 1] == 'Z')) {
        ptrdiff_t base;
        if (!get_base(var_info(vindex), base)) {
          continue;
...
//
AP_Param *AP_Param::find_by_index(uint16_t idx, enum ap_var_type *ptype,
                                  ParamToken *token) {
  AP_Param *ap;
  uint16_t count = 0;
  for (ap = AP_Param::first(token, ptype); ap && count < idx;
...
AP_Param *AP_Param::find_by_name(const char *name, enum ap_var_type *ptype,
                                 ParamToken *token) {
  AP_Param *ap;
  for (ap = AP_Param::first(token, ptype);
    ...
    /*
      ...
      This check avoids us traversing down into most groups, saving a lot of
      calls to copy_name_token()
     */
...
/*
  Find a variable by pointer, returning key. This is used for loading pointer
  variables
*/
...
bool AP_Param::find_key_by_pointer(const void *ptr, uint16_t &key) {
  for (uint16_t i = 0; i < _num_vars; i++) {
    ...
    if ((info.flags & AP_PARAM_FLAG_POINTER) && ptr == *(void **)info.ptr) {
      key = info.key;
      return true;
    }
    ptrdiff_t offset = 0;
...
*/
bool AP_Param::get_param_by_index(void *obj_ptr, uint8_t idx,
                                  ap_var_type old_ptype, void *pvalue) {
  uint16_t key;
...
void AP_Param::notify() const {
  ...
  const struct AP_Param::Info *info =
      find_var_info(&group_element, ginfo, group_nesting, &idx);
  if (info == nullptr) {
...
void AP_Param::save_sync(bool force_save, bool send_to_gcs) {
  ...
  if (phdr.type == AP_PARAM_INT8 && ginfo != nullptr &&
  ...
  if (scan(&phdr, &ofs)) {
    ...
    eeprom_write_check(ap, ofs + sizeof(phdr),
                       type_size((enum ap_var_type)phdr.type));
    if (send_to_gcs) {
  ...
  if (phdr.type <= AP_PARAM_FLOAT) {
    ...
    if (!force_save && (phdr.type != AP_PARAM_INT32 &&
  ...
  if (ofs + type_size((enum ap_var_type)phdr.type) + 2 * sizeof(phdr) >=
      _storage.size()) {
...
void AP_Param::save(bool force_save) {
  ...
  if (save_queue.peek(p2) && p2.param == this && p2.force_save == force_save) {
...
bool AP_Param::load(void) {
  ...
  if (!scan(&phdr, &ofs)) {
    ...
    if (ginfo != nullptr) {
      ...
      set_value((enum ap_var_type)phdr.type,
                (void *)(base + ginfo->offset + group_offset),
  ...
  // found it
  _storage.read_block(ap, ofs + sizeof(phdr),
                      type_size((enum ap_var_type)phdr.type));
  return true;
}
  ...
  for (uint8_t i = 0; (type = group_info[i].type) != AP_PARAM_NONE; i++) {
    if (type <= AP_PARAM_FLOAT) {
      ...
      const float default_val =
          get_default_value((const AP_Param *)ptr, group_info[i]);
      ((AP_Vector3f *)ptr)
          ->set(Vector3f{default_val, default_val, default_val});
    }
  }
}
...
bool AP_Param::load_all() {
  ...
  if (!registered_save_handler) {
    ...
    hal.scheduler->register_io_process(
        FUNCTOR_BIND((&save_dummy), &AP_Param::save_io_handler, void));
  }
  ...
  while (ofs < _storage.size()) {
    ...
    if (info != nullptr) {
      _storage.read_block(ptr, ofs + sizeof(phdr),
                          type_size((enum ap_var_type)phdr.type));
    }
...
#if AP_FILESYSTEM_FILE_READING_ENABLED
void AP_Param::load_defaults_file_from_filesystem(const char *default_file,
                                                  bool last_pass) {
...
void AP_Param::load_defaults_file_from_romfs(const char *default_file,
                                             bool last_pass) {
  ...
  const uint8_t *text =
      AP_ROMFS::find_decompress(trimmed_filename, string_length);
  if (text == nullptr) {
    return;
  }
 ...
 */
void AP_Param::load_object_from_eeprom(const void *object_pointer,
                                       const struct GroupInfo *group_info) {
  struct Param_header phdr;
  uint16_t key;
    ...
    if (group_info[i].type == AP_PARAM_GROUP) {
      ...
      if (ginfo != nullptr) {
        load_object_from_eeprom(
            (void *)(((ptrdiff_t)object_pointer) + new_offset), ginfo);
      }
    }
    uint16_t ofs = sizeof(AP_Param::EEPROM_header);
    while (ofs < _storage.size()) {
      ...
      if (get_key(phdr) == key) {
        ...
        if (info != nullptr) {
          if ((ptrdiff_t)ptr ==
              ((ptrdiff_t)object_pointer) + group_info[i].offset) {
            _storage.read_block(ptr, ofs + sizeof(phdr),
                                type_size((enum ap_var_type)phdr.type));
            break;
          }
...
/// as needed
AP_Param *AP_Param::next_group(const uint16_t vindex,
                               const struct GroupInfo *group_info,
                               bool *found_current, const uint32_t group_base,
                               const uint8_t group_shift,
                               const ptrdiff_t group_offset, ParamToken *token,
                               enum ap_var_type *ptype, bool skip_disabled,
                               float *default_val) {
  enum ap_var_type type;
  for (uint8_t i = 0;
       (type = (enum ap_var_type)group_info[i].type) != AP_PARAM_NONE; i++) {
    ...
    if (type == AP_PARAM_GROUP) {
      ...
      ap = next_group(vindex, ginfo, found_current,
                      group_id(group_info, group_base, i, group_shift),
                      group_shift + _group_level_shift, new_offset, token,
                      ptype, skip_disabled, default_val);
      if (ap != nullptr) {
        return ap;
      }
    } else {
      if (*found_current) {
        ...
        AP_Param *ret =
            (AP_Param *)(base + group_info[i].offset + group_offset);

        if (skip_disabled && _hide_disabled_groups &&
...
AP_Param *AP_Param::next(ParamToken *token, enum ap_var_type *ptype,
                         bool skip_disabled, float *default_val) {
  uint16_t i = token->key;
  ...
  if (type == AP_PARAM_VECTOR3F && token->idx < 3) {
    ...
    AP_Param *ret = (AP_Param *)(((token->idx - 1u) * sizeof(float)) +
                                 (ptrdiff_t)var_info(i).ptr);
#if AP_PARAM_DEFAULTS_ENABLED
    if (default_val != nullptr) {
  ...
  for (; i < _num_vars; i++) {
    ...
    if (type == AP_PARAM_GROUP) {
      ...
      AP_Param *ap = next_group(i, group_info, &found_current, 0, 0, 0, token,
                                ptype, skip_disabled, default_val);
      if (ap != nullptr) {
        return ap;
      }
...
AP_Param *AP_Param::next_scalar(ParamToken *token, enum ap_var_type *ptype,
  ...
  AP_Param *ap;
  enum ap_var_type type;
  while ((ap = next(token, &type, true, default_val)) != nullptr &&
         type > AP_PARAM_FLOAT)
    ;
 ...
 */
bool AP_Param::find_old_parameter(const struct ConversionInfo *info,
                                  AP_Param *value) {
  ...
  _storage.read_block(value, pofs + sizeof(header),
                      type_size((enum ap_var_type)header.type));
  return true;
}
...
void AP_Param::convert_old_parameters(
    const struct ConversionInfo *conversion_table, uint8_t table_size,
    uint8_t flags) {
  convert_old_parameters_scaled(conversion_table, table_size, 1.0f, flags);
}

// convert old vehicle parameters to new object parameters with scaling -
// assumes all parameters will have the same scaling factor
void AP_Param::convert_old_parameters_scaled(
    const struct ConversionInfo *conversion_table, uint8_t table_size,
    float scaler, uint8_t flags) {
...
// is_top_level: Is true if the class had its own top level key, param_key. It
// is false if the class was a subgroup
void AP_Param::convert_class(uint16_t param_key, void *object_pointer,
                             const struct AP_Param::GroupInfo *group_info,
                             uint16_t old_index, bool is_top_level,
                             bool recurse_sub_groups) {
  ...
  for (uint8_t i = 0; group_info[i].type != AP_PARAM_NONE; i++) {
    ...
    if (group_shift != 0 && idx == 0) {
      // Note: Index 0 is treated as 63 for group bit shifting purposes. See
      // group_id()
      idx = 63;
    }
    ...
    if (info.type == AP_PARAM_GROUP) {
      ...
      if (recurse_sub_groups) {
        // Only recurse once
        convert_class(param_key,
                      (uint8_t *)object_pointer + group_info[i].offset,
                      get_group_info(group_info[i]), idx, false, false);
      }
      continue;
    }
    ...
    AP_Param *ap2 =
        (AP_Param *)(group_info[i].offset + (uint8_t *)object_pointer);
...
void AP_Param::convert_g2_objects(const void *g2,
...
void AP_Param::convert_toplevel_objects(
    const TopLevelObjectConversion conversions[], uint8_t num_conversions) {
  for (uint8_t i = 0; i < num_conversions; i++) {
...
*/
bool AP_Param::_convert_parameter_width(ap_var_type old_ptype,
                                        float scale_factor, bool bitmask) {
...
void AP_Param::set_float(float value, enum ap_var_type var_type) {
  ...
  if (var_type == AP_PARAM_FLOAT) {
    ...
    if (value < 0)
      rounding_addition = -rounding_addition;
...
bool AP_Param::parse_param_line(char *line, char **vname, float &value,
  ...
  // EEPROM. For details, see:
  // https://github.com/ArduPilot/ardupilot/issues/15579
...
bool AP_Param::count_defaults_in_file(const char *filename,
    ...
    if (!vp) {
      if (last_pass) {
        ...
        ::printf("Ignored unknown param %s in defaults file %s\n", pname,
                 filename);
        hal.console->printf("Ignored unknown param %s in defaults file %s\n",
                            pname, filename);
#endif
      }
      done_all = false;
...
bool AP_Param::load_defaults_file(const char *filename, bool last_pass) {
  ...
  for (char *pname = strtok_r(mutable_filename, ",", &saveptr);
       pname != nullptr; pname = strtok_r(nullptr, ",", &saveptr)) {
 ...
 */
bool AP_Param::count_param_defaults(const volatile char *ptr, int32_t length,
                                    uint16_t &count) {
  count = 0;
 ...
 */
void AP_Param::load_param_defaults(const volatile char *ptr, int32_t length,
                                   bool last_pass) {
  delete[] param_overrides;
  ...
  num_param_overrides = num_defaults;
       ...
       // defined(HAL_HAVE_AP_ROMFS_EMBEDDED_H)
...
void AP_Param::load_embedded_param_defaults(bool last_pass) {
  load_param_defaults(param_defaults_data.data, param_defaults_data.length,
                      last_pass);
}
#endif // AP_PARAM_MAX_EMBEDDED_PARAM > 0
 ...
 */
float AP_Param::get_default_value(const AP_Param *vp,
                                  const struct GroupInfo &info) {
  for (uint16_t i = 0; i < num_param_overrides; i++) {
...
void AP_Param::send_parameter(const char *name, enum ap_var_type var_type,
                              uint8_t idx) const {
  ...
  // for vectors we need to send 3 messages. Note that we also come here for the
  // case of a set of the first element of a AP_Vector3f. This happens as the
  // ap->save() call can't distinguish between a vector and scalar save. It
  // means that setting first element of a vector via MAVLink results in sending
  // all 3 elements to the GCS
#if HAL_GCS_ENABLED
...
uint16_t AP_Param::count_parameters(void) {
  ...
  if (_parameter_count != 0 && _count_marker == _count_marker_done) {
   ...
   */
  uint8_t limit = 4;
  while ((_parameter_count == 0 || _count_marker != _count_marker_done) &&
         limit--) {
    AP_Param *vp;
    ...
    for (vp = AP_Param::first(&token, nullptr); vp != nullptr;
 ...
 */
void AP_Param::set_defaults_from_table(
    const struct defaults_table_struct *table, uint8_t count) {
...
void AP_Param::show(const AP_Param *ap, const char *s, enum ap_var_type type,
...
void AP_Param::show_all(AP_HAL::BetterStream *port, bool showKeyValues) {
  ...
  for (ap = AP_Param::first(&token, &type, &default_value); ap;
       ap = AP_Param::next_scalar(&token, &type, &default_value)) {
    if (showKeyValues) {
      ::printf("Key %u: Index %u: GroupElement %u : Default %f  :",
               (unsigned)var_info(token.key).key, (unsigned)token.idx,
               (unsigned)token.group_element, default_value);
      default_value = NaNf;
    }
...
/*
  ...
  The layout we create is as follows:
    - a top level Info with the given prefix, using one of the 10 possible slots
  in _var_info_dynamic
    - a dynamically allocated GroupInfo table, never freed, of size
  (num_params+2)
    - the GroupInfo table has an initial AP_Int32 hidden entry with a 32 bit CRC
  of the prefix
...
bool AP_Param::add_table(uint8_t _key, const char *prefix, uint8_t num_params) {
  ...
  const int32_t crc =
      int32_t(crc32_small(0, (const uint8_t *)prefix, strlen(prefix)));
  int32_t current_crc;
  ...
  if (_var_info_dynamic == nullptr) {
    _var_info_dynamic =
        (Info *)calloc(AP_PARAM_MAX_DYNAMIC, sizeof(struct Info));
    if (_var_info_dynamic == nullptr) {
      return false;
    }
...
/*
 Load an AP_Int32 variable from EEPROM using top level key and group element.
 Used to confirm a key in add_table()
*/
bool AP_Param::load_int32(uint16_t key, uint32_t group_element,
                          int32_t &value) {
  struct Param_header phdr;
 ...
 */
bool AP_Param::add_param(uint8_t _key, uint8_t param_num, const char *pname,
                         float default_value) {
  if (_var_info_dynamic == nullptr) {
  ...
  const int fullNameLength =
      hal.util->snprintf(fullName, sizeof(fullName), "%s%s", info.name, pname);
  ...
  if ((existingParam != nullptr) &&
      ((existingType != AP_PARAM_FLOAT) || ((AP_Float *)existingParam != &p))) {
    // There is a existing parameter with this name (which is not this parameter
    // from a previous script run)
    return false;
  }
