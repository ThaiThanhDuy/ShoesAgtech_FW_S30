...
#define SERVO_MAX                                                              \
  4500 // This value represents 45 degrees and is just an arbitrary
...
const AP_Param::GroupInfo AP_MotorsUGV::var_info[] = {
    ...
    // @Description: This selects the output PWM type as regular PWM, OneShot,
    // Brushed motor support using PWM (duty cycle) with separated direction
    // signal, Brushed motor support with separate throttle and direction PWM
    // (duty cycle)
    // @Values:
    // 0:Normal,1:OneShot,2:OneShot125,3:BrushedWithRelay,4:BrushedBiPolar,5:DShot150,6:DShot300,7:DShot600,8:DShot1200
    // @User: Advanced
    // @RebootRequired: True
    ...
    // @Description: Throttle minimum percentage the autopilot will apply. This
    // is useful for handling a deadzone around low throttle and for preventing
    // internal combustion motors cutting out during missions.
    // @Units: %
    // @Range: 0 20
    // @Increment: 1
    // @User: Standard
    ...
    // @Description: Throttle maximum percentage the autopilot will apply. This
    // can be used to prevent overheating an ESC or motor on an electric rover
    // @Units: %
    // @Range: 30 100
    // @Increment: 1
    // @User: Standard
    ...
    // @Description: Throttle slew rate as a percentage of total range per
    // second. A value of 100 allows the motor to change over its full range in
    // one second.  A value of zero disables the limit.  Note some NiMH powered
    // rovers require a lower setting of 40 to reduce current demand to avoid
    // brownouts.
    // @Units: %/s
    // @Range: 0 1000
    // @Increment: 1
    ...
    // @Description: Speed above which steering is scaled down when using
    // regular steering/throttle vehicles.  zero to disable speed scaling
    // @Units: m/s
    // @Range: 0 10
    // @User: Advanced
    ...
    // @Description: Steering vs Throttle priorisation.  Higher numbers
    // prioritise steering, lower numbers prioritise throttle.  Only valid for
    ...
    // @Description: The angle between steering's middle position and maximum
    // position when using vectored thrust (boats only)
    // @Units: deg
    // @Range: 0 90
    // @User: Standard
    ...
    // @Description: Thrust Asymmetry. Used for skid-steering. 2.0 means your
    // motors move twice as fast forward than they do backwards.
    // @Range: 1.0 10.0
    // @User: Advanced
    ...
    // @Description: For reversible motors that need a delay before they can
    // change direction. When greater than zero the throttle will go to zero for
    // this amount of time before outputting the new throttle when the demanded
...
bool AP_MotorsUGV::get_legacy_relay_index(int8_t &index1, int8_t &index2,
                                          int8_t &index3,
                                          int8_t &index4) const {
...
// used by manual mode to avoid bad steering behaviour during transitions from
// forward to reverse same as private slew_limit_throttle method (see below) but
// does not update throttle state
float AP_MotorsUGV::get_slew_limited_throttle(float throttle, float dt) const {
  ...
  return constrain_float(throttle, _throttle_prev - throttle_change_max,
                         _throttle_prev + throttle_change_max);
}
...
bool AP_MotorsUGV::has_sail() const {
  return SRV_Channels::function_assigned(SRV_Channel::k_mainsail_sheet) ||
         SRV_Channels::function_assigned(SRV_Channel::k_wingsail_elevator) ||
         SRV_Channels::function_assigned(SRV_Channel::k_mast_rotation);
}
...
void AP_MotorsUGV::output(bool armed, float ground_speed, float dt) {
  ...
  // clear limit flags
  // output_ methods are responsible for setting them to true if required on
  // each iteration
  limit.steer_left = limit.steer_right = limit.throttle_lower =
...
// test steering or throttle output as a percentage of the total (range -100 to
// +100) used in response to DO_MOTOR_TEST mavlink command
...
//  returns true if checks pass, false if they fail.  report should be true to
//  send text messages to GCS
bool AP_MotorsUGV::pre_arm_check(bool report) const {
  const bool have_throttle =
      SRV_Channels::function_assigned(SRV_Channel::k_throttle);
  const bool have_throttle_left =
      SRV_Channels::function_assigned(SRV_Channel::k_throttleLeft);
  const bool have_throttle_right =
      SRV_Channels::function_assigned(SRV_Channel::k_throttleRight);

  // check that there's defined outputs, inc scripting and sail
  if (!have_throttle_left && !have_throttle_right && !have_throttle &&
  ...
  if (have_throttle_left != have_throttle_right) {
    if (report) {
      GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL,
                    "PreArm: check skid steering config");
    }
    return false;
  }
  // check if only one of throttle or steering outputs has been configured, if
  // has a sail allow no throttle
  if ((has_sail() || have_throttle) !=
      SRV_Channels::function_assigned(SRV_Channel::k_steering)) {
    if (report) {
      GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL,
  ...
  for (uint8_t i = 0; i < _motors_num; i++) {
    ...
    if (!SRV_Channels::function_assigned(function)) {
      if (report) {
        GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL,
                      "PreArm: servo function %u unassigned", function);
      }
      return false;
    }
  }
  ...
  if ((_pwm_type == PWMType::BRUSHED_WITH_RELAY) && (relay != nullptr)) {
    ...
    const RelayTable relay_table[] = {
        {have_throttle || have_throttle_left ||
             (SRV_Channels::function_assigned(SRV_Channel::k_motor1) &&
              (_motors_num >= 1)),
         AP_Relay_Params::FUNCTION::BRUSHED_REVERSE_1},
        {have_throttle_right ||
             (SRV_Channels::function_assigned(SRV_Channel::k_motor2) &&
              (_motors_num >= 2)),
         AP_Relay_Params::FUNCTION::BRUSHED_REVERSE_2},
        {SRV_Channels::function_assigned(SRV_Channel::k_motor3) &&
             (_motors_num >= 3),
         AP_Relay_Params::FUNCTION::BRUSHED_REVERSE_3},
        {SRV_Channels::function_assigned(SRV_Channel::k_motor4) &&
             (_motors_num >= 4),
         AP_Relay_Params::FUNCTION::BRUSHED_REVERSE_4},
    };

    for (uint8_t i = 0; i < ARRAY_SIZE(relay_table); i++) {
      if (relay_table[i].output_assigned &&
          !relay->enabled(relay_table[i].fun)) {
        if (report) {
          GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL,
                        "PreArm: relay function %u unassigned",
                        uint8_t(relay_table[i].fun));
        }
        return false;
      }
    }
...
void AP_MotorsUGV::setup_pwm_type() {
  ...
  for (uint8_t i = 0; i < _motors_num; i++) {
    _motor_mask |= SRV_Channels::get_output_channel_mask(
  ...
  case PWMType::ONESHOT125:
    hal.rcout->set_output_mode(_motor_mask,
                               AP_HAL::RCOutput::MODE_PWM_ONESHOT125);
    break;
  ...
  case PWMType::DSHOT150:
    hal.rcout->set_output_mode(_motor_mask,
                               AP_HAL::RCOutput::MODE_PWM_DSHOT150);
    break;
  case PWMType::DSHOT300:
    hal.rcout->set_output_mode(_motor_mask,
                               AP_HAL::RCOutput::MODE_PWM_DSHOT300);
    break;
  case PWMType::DSHOT600:
    hal.rcout->set_output_mode(_motor_mask,
                               AP_HAL::RCOutput::MODE_PWM_DSHOT600);
    break;
...
void AP_MotorsUGV::setup_omni() {
  ...
  case FRAME_TYPE_OMNIX:
    _motors_num = 4, add_omni_motor(0, 1.0f, -1.0f, -1.0f);
...
void AP_MotorsUGV::add_omni_motor(int8_t motor_num, float throttle_factor,
                                  float steering_factor, float lateral_factor) {
...
void AP_MotorsUGV::add_omni_motor_num(int8_t motor_num) {
  ...
  if (motor_num >= 0 && motor_num < AP_MOTORS_NUM_MOTORS_MAX) {
    uint8_t chan;
    SRV_Channel::Function function =
        SRV_Channels::get_motor_function(motor_num);
    SRV_Channels::set_aux_channel_default(function, motor_num);
    if (!SRV_Channels::find_channel(function, chan)) {
      GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Motors: unable to setup motor %u",
                    motor_num);
    }
  }
}

// disable omni motor and remove all throttle, steering and lateral factor for
// this motor
...
void AP_MotorsUGV::output_regular(bool armed, float ground_speed,
  ...
  if (armed) {
    if (_scale_steering) {
      ...
      if (have_vectored_thrust()) {
        ...
        const float vector_angle_max_rad =
            radians(constrain_float(_vector_angle_max, 0.0f, 90.0f));
        const float steering_norm_lim =
            fabsf(throttle_norm * tanf(vector_angle_max_rad));
        ...
        if (!is_zero(throttle_norm)) {
          ...
          if (fabsf(steering_angle_rad) > vector_angle_max_rad) {
            steering_angle_rad =
                constrain_float(steering_angle_rad, -vector_angle_max_rad,
                                vector_angle_max_rad);
            limit.steer_right = true;
            limit.steer_left = true;
          }
        ...
        // scale steering down as speed increase above MOT_SPD_SCA_BASE (1 m/s
        // default)
        if (is_positive(_speed_scale_base) &&
            (fabsf(ground_speed) > _speed_scale_base)) {
          steering *= (_speed_scale_base / fabsf(ground_speed));
        } else {
          // regular steering rover at low speed so set limits to stop I-term
          // build-up in controllers
          if (!have_skid_steering()) {
            limit.steer_left = true;
            limit.steer_right = true;
          }
  ...
  // we do this here because vectored thrust or speed scaling may have reduced
  // steering request
...
void AP_MotorsUGV::output_skid_steering(bool armed, float steering,
  ...
  // sanitize values for asymmetry of thrust, mixer assumes forward thrust is
  // always larger than reverse
  ...
  if (throttle_scaled < best_steering_throttle) {
    ...
    // full range available, throttle can always be lowered down to
    // best_steering_throttle
  ...
  // All throttle or all steering will now fit, check if they will both fit
  // together
  ...
  const float saturation_value =
      MAX(max_output, min_output / lower_throttle_limit);
  if (saturation_value > 1.0f) {
    ...
    const float str_thr_mix =
        constrain_float(_steering_throttle_mix, 0.0f, 1.0f);
    const float fair_scaler = 1.0f / saturation_value;
    if (str_thr_mix >= 0.5f) {
      // prioritise steering over throttle
      steering_scaled *=
          linear_interpolate(fair_scaler, 1.0f, str_thr_mix, 0.5f, 1.0f);
      if (throttle_scaled >= best_steering_throttle) {
      ...
      throttle_scaled *=
          linear_interpolate(fair_scaler, 1.0f, 0.5f - str_thr_mix, 0.0f, 0.5f);
      ...
      if (throttle_scaled >= best_steering_throttle) {
        ...
        // constrained by lower limit
        steering_scaled =
            (throttle_scaled - lower_throttle_limit) * steering_sign;
      }
    }
...
void AP_MotorsUGV::output_omni(bool armed, float steering, float throttle,
  ...
  if (armed) {
    ...
    for (uint8_t i = 0; i < _motors_num; i++) {
      ...
      output_throttle(SRV_Channels::get_motor_function(i),
    ...
    if (_disarm_disable_pwm) {
      for (uint8_t i = 0; i < _motors_num; i++) {
        SRV_Channels::set_output_limit(SRV_Channels::get_motor_function(i),
                                       SRV_Channel::Limit::ZERO_PWM);
      }
    } else {
      for (uint8_t i = 0; i < _motors_num; i++) {
...
// output throttle value to main throttle channel, left throttle or right
// throttle.  throttle should be scaled from -100 to 100
void AP_MotorsUGV::output_throttle(SRV_Channel::Function function,
  ...
  if (function != SRV_Channel::k_throttle &&
      ...
      function != SRV_Channel::k_motor1 && function != SRV_Channel::k_motor2 &&
      function != SRV_Channel::k_motor3 && function != SRV_Channel::k_motor4) {
    return;
  }
...
void AP_MotorsUGV::set_limits_from_input(bool armed, float steering,
                                         float throttle) {
...
float AP_MotorsUGV::get_scaled_throttle(float throttle) const {
  ...
  if (_throttle_min > 0) {
    if (is_negative(throttle)) {
      throttle =
          -_throttle_min + (throttle * ((100.0f - _throttle_min) * 0.01f));
    } else {
      throttle =
          _throttle_min + (throttle * ((100.0f - _throttle_min) * 0.01f));
    }
  ...
  if (is_zero(_thrust_curve_expo) || (_thrust_curve_expo > 1.0f) ||
      (_thrust_curve_expo < -1.0f)) {
    return throttle;
  }
  ...
  return 100.0f * sign *
         ((_thrust_curve_expo - 1.0f) +
          safe_sqrt((1.0f - _thrust_curve_expo) * (1.0f - _thrust_curve_expo) +
                    4.0f * _thrust_curve_expo * fabsf(throttle_pct))) /
         (2.0f * _thrust_curve_expo);
}

// use rate controller to achieve desired throttle
float AP_MotorsUGV::get_rate_controlled_throttle(SRV_Channel::Function function,
                                                 float throttle, float dt) {
  // require non-zero dt
  if (!is_positive(dt)) {
    return throttle;
  ...
  if ((function == SRV_Channel::k_throttleLeft) &&
      _rate_controller.enabled(0)) {
    return _rate_controller.get_rate_controlled_throttle(0, throttle, dt);
  }

  // rate control right throttle
  if ((function == SRV_Channel::k_throttleRight) &&
      _rate_controller.enabled(1)) {
...
// returns true if the configured PWM type is digital and should have fixed
// endpoints
 ...
 */
void AP_MotorsUGV::ReverseThrottle::output(SRV_Channel::Function function,
                                           float throttle, float delay) {
...
namespace AP {
AP_MotorsUGV *motors_ugv() { return AP_MotorsUGV::get_singleton(); }
} // namespace AP
