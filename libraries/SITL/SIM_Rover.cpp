...
void SimRover::update_ackermann_or_skid(const struct sitl_input &input,
  ...
  // --- LOGIC GIẢ LẬP ĐỘNG LỰC HỌC PITCH SẠ HÀNG (S16/S30) NÂNG CẤP ---
  enum class PitchSimState {
    IDLE,
    DECREASING, // Mô phỏng sập phao trước / va chạm chướng ngại vật
    INCREASING // Phục hồi trạng thái cân bằng nhờ lực nổi composite
  };

  static PitchSimState pitch_state = PitchSimState::DECREASING;
  static const float PITCH_LIMIT_DEG =
      -8.0f; // Ngưỡng chúi mũi sâu để test vi phạm
  static const float PITCH_RESTORE_DEG = 0.0f; // Trạng thái phẳng mặt nước

  // Tăng mạnh gia tốc góc để vượt qua lỗi underflow số thực của bộ lọc LPF
  static const float PITCH_ACCEL_RATE =
      1.5f; // rad/s^2 (Tốc độ tăng tốc độ quay)
  static float current_pitch_rate =
      0.0f; // Biến tĩnh lưu trữ vận tốc góc tích lũy (rad/s)
  ...
  case PitchSimState::DECREASING:
    if (current_pitch_deg > PITCH_LIMIT_DEG) {
      // Vận tốc góc âm dần liên tục theo thời gian thực
      current_pitch_rate -= PITCH_ACCEL_RATE * delta_time;
    } else {
      // Đã chạm đáy nguy hiểm, chuyển trạng thái giật ngược góc
  ...
  // Giới hạn (Saturation) tốc độ quay vật lý tối đa để bảo vệ mô hình ma trận
  // DCM
  current_pitch_rate = constrain_float(current_pitch_rate, -0.8f, 0.8f);

  // Ép xung gyro trục Y trực tiếp vào cấu trúc trạng thái của SITL
  gyro = Vector3f(0.0f, current_pitch_rate, radians(yaw_rate));
  // ------------------------------------------------------------------
  // --------------------------------------------------
  // update attitude
