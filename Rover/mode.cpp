...
void Mode::navigate_to_waypoint() {
  ...
  // THROTTLE LOGIC
  const bool bst_enabled = (rover.g.bst_enable.get() == 1);
  const uint32_t now_ms = AP_HAL::millis();
  const bool is_auto = (rover.control_mode == &rover.mode_auto);
  const bool is_armed = hal.util->get_soft_armed();

  // 1. INITIAL SPEED DATA (INPUT)
  ...
  if (bst_enabled) {
    ...
    rem_ms = (is_in_boost_window) ? static_cast<uint32_t>(STARTBOOST_MS) -
                                        (now_ms - boost_start_ms)
                                  : 0U;
  }
  ...
  if (rover.g.bst_pitch_en.get() == 1) {
    ...
    if (!is_auto || !is_armed) {
      ...
      if (is_pitch_bad) {
        // Áp trần giới hạn tốc độ dựa trên wp_speed_max để kiểm soát động năng
        const float max_allowed_speed = wp_speed_max * pitch_scale;
        if (desired_speed > max_allowed_speed) {
          desired_speed = max_allowed_speed;
        }
        ...
        if (!_pitch_warning_sent) {
          gcs().send_text(MAV_SEVERITY_CRITICAL,
                          "[BST] PITCH DANGER! Ang:%.1fdeg Acc:%.1fdeg/s2 -> "
                          "LIMIT TO %.2f m/s",
                          static_cast<double>(pitch_deg),
        ...
        if (now_ms - _pitch_safe_start_ms >= recovery_delay_ms) {
          GCS_SEND_TEXT(MAV_SEVERITY_WARNING,
                        "[BST] Pitch & Inertia Safe - Resuming");
          _pitch_warning_sent = false;
          _pitch_safe_start_ms = 0U;
        } else {
          // Duy trì giới hạn tốc độ trong thời gian trễ phục hồi
  ...
  const bool log_enabled = (rover.g.bst_log_en.get() == 1);
  if (log_enabled && (now_ms - last_dbg_ms >= 1000U)) {
    last_dbg_ms = now_ms;
    float actual_speed = 0.0f;
    attitude_control.get_forward_speed(actual_speed);

    GCS_SEND_TEXT(
        MAV_SEVERITY_INFO, "[BST] TGT:%.1f ACT:%.1f MODE:%s rem:%ums EN:%d",
        static_cast<double>(desired_speed), static_cast<double>(actual_speed),
        apply_minspeed
            ? (desired_speed >= wp_speed_instant + 0.01f ? "MIN" : "OK")
            : "NORM",
        static_cast<unsigned>(rem_ms), static_cast<int>(bst_enabled));
  }
